...
test("client selectors match the HTML and preserve Alpha UX constraints", async () => {
  ...
  assert.match(javascript, /<div class="challenge-hist">/);
  assert.match(javascript, /collectorCrowd/);
  assert.match(html, /class="challenge-hero"/);
  assert.match(html, /id="daily-challenge-date"/);
  assert.match(html, /id="daily-challenge-score"/);
  assert.match(html, /id="daily-challenge-recap"/);
  assert.match(html, /id="daily-challenge-party"/);
  assert.match(html, /id="daily-challenge-title">מי אסף היום הכי הרבה קלפים של</);
  assert.match(javascript, /מי אסף היום הכי הרבה קלפים של/);
  assert.match(javascript, /dailyChallengeParty\.textContent = partyDisplayName\(challenge\?\.targetPartyId\)/);
  ...
  assert.match(css, /\.growth-grid \.daily-challenge-desk #daily-challenge-title,\s*\.growth-grid \.daily-challenge-desk #daily-challenge-party/);
  assert.match(css, /#daily-challenge-title,\s*\.growth-grid \.daily-challenge-desk #daily-challenge-party \{[^}]*Noto Serif Hebrew/);
  assert.match(css, /\.growth-grid \.daily-challenge-desk #daily-challenge-date \{\s*margin: 0;\s*color: var\(--civic\)/);
  ...
  assert.match(javascript, /visible-sets-3/);
  assert.match(warmup, /visible-sets-3/);
  assert.match(warmup, /const PLAYABLE_ART_SET_IDS = \["party-leaders", "party-slot-2", "set-5"\]/);
  assert.doesNotMatch(warmup, /\.slice\(0, 12\)/);
