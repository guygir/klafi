...
test("client selectors match the HTML and preserve Alpha UX constraints", async () => {
  ...
  assert.doesNotMatch(html, /id="dialog-whatsapp"/);
  assert.doesNotMatch(javascript, /https:\/\/wa\.me\/\?text=/);
  ...
  assert.match(html, /id="dialog-copies"/);
  ...
  assert.match(javascript, /dialogCopies\.textContent = showCopies \? `×\$\{ownedCount\}`/);
  ...
  assert.match(html, /card-surface-1/);
  ...
  assert.doesNotMatch(html, /id="dialog-instagram"/);
  assert.match(html, /data-debug-only/);
  assert.match(javascript, /function applyVisualConfig/);
  assert.doesNotMatch(javascript, /function shareToInstagram/);
