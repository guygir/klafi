...
The committed `.env` is the local/private-repo default (`kalpi` / `change-me` on `127.0.0.1`). It is not a cloud secret. Live Supabase credentials live only in Vercel env. Quiz stays off until `QUIZ_ENABLED=1`.
